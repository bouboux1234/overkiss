...
def generate_local_serverX(
    host: str | None = None,
    name: str | None = "Somfy TaHoma Developer Mode Coonexoon JAILBREAK v1.14 (local API)",
    manufacturer: str | None = "Somfy",
    configuration_url: str | None = None,
) -> OverkizServer:
    """Generate OverkizServer object for local API."""
    LOGGER.debug("CONFIGURING LOCAL INTEGRATION - generate_local_server")
    return OverkizServer(
        name=name,
        endpoint=f"https://172.20.199.99:8443/enduser-mobile-web/1/enduserAPI/",
        manufacturer=manufacturer,
        configuration_url=f"https://172.20.99.199:8443/enduser-mobile-web/1/enduserAPI/"
    )

#//ALIBABA40VOLEUR
LOCAL = "local"
LOCAL_HUB = {LOCAL: generate_local_serverX()}
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_validate_input(self, user_input: dict[str, Any]) -> dict[str, Any]:
        ...
        # Local API
        if self._api_type == APIType.LOCAL or 1==1:
            ...
            uuid="1234-5678"
            gateway_id="0825-1222-7691" 
            token='ALIBABA40VOLEUR'
            token='ALIBABA' #2023 ?????            
            ...
            server = SUPPORTED_SERVERS[self._server]
            #uuid="67ed7ab8-346c-41df-a674-f9ff9ebd743e"

            local_client = OverkizClient(
                username="Bearer", #new 04 2023 :(((
                password="ALIBABA",
                token=token,
                session=session,
                server=generate_local_server(host=user_input[CONF_HOST]),
            )
            LOGGER.debug("SERVEUR gateway %s", str(generate_local_server(host=user_input[CONF_HOST])) ) 
    ...
    def __init__(self) -> None:
        ...
        #DEFAULT_SERVER
        self._host = "172.20.99.99"


    async def async_validate_inputXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX2(self, user_input: dict[str, Any]) -> dict[str, Any]:
        """Validate user credentials."""
        username = user_input[CONF_USERNAME]
        password = user_input[CONF_PASSWORD]
        ...
        if self._api_type == APIType.LOCAL:
            ...
            # Somfy (self-signed) SSL cert uses the wrong common name
            session = async_create_clientsession(self.hass, verify_ssl=False)
            user_input[CONF_HOST]='172.20.99.99'
